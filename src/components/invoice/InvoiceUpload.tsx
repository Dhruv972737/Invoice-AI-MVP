...
export default function InvoiceUpload({ onUploadComplete }: InvoiceUploadProps) {
  ...
  const processFile = async (file: File, index: number) => {
    try {
      ...
      if (dbError) {
        console.error('Supabase insert(invoices) error:', dbError);
        throw dbError;
      }
  ...
  const simulateAIProcessing = async (invoiceId: string, index: number, file: File) => {
    try {
      ...
      if (!(resp as Response).ok || !data?.success) {
        ...
        throw new Error(data?.error || `Failed to enqueue job (status ${(resp as Response).status})`);
      }
      ...
      showToast('info', 'Queued for Processing', 'Invoice has been queued and will be processed shortly.');
