...
import { Eye, EyeOff, Mail, Lock, User, FileText, Brain } from 'lucide-react';
...
export default function AuthPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (error) {
          ...
          if (error.message?.includes('Invalid login credentials') || 
            ...
            showToast('error', 'Invalid Credentials', 'Invalid email or password. Please check your credentials or sign up if you don\'t have an account.');
          } else {
        ...
        if (error) {
          ...
          if (error.message.includes('User already registered')) {
            showToast('info', 'Account exists', 'This email is already registered. Try signing in instead.');
            setIsLogin(true);
          } else if (error.message.includes('Password should be at least')) {
            showToast('error', 'Weak Password', 'Password should be at least 6 characters long.');
          } else if (error.message.includes('Unable to validate email address')) {
            showToast('error', 'Invalid Email', 'Please enter a valid email address.');
          } else if (error.message.includes('Signup is disabled')) {
            showToast('error', 'Signup Disabled', 'New user registration is currently disabled.');
          } else {
            showToast('error', 'Sign Up Error', error.message || 'Failed to create account. Please try again.');
          }
        } else {
      ...
      showToast('error', 'Authentication Error', error.message || 'An unexpected error occurred.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Don't show toast immediately - let the OAuth flow complete
    } catch (error: any) {
      console.error('Google OAuth error:', error);
      if (error.message?.includes('Supabase configuration')) {
        showToast('error', 'Configuration Error', 'Supabase is not properly configured. Please check your environment variables.');
      } else if (error.message?.includes('refused to connect')) {
        showToast('error', 'Connection Error', 'Cannot connect to Supabase. Please check your network connection and Supabase URL.');
      } else if (error.message?.includes('popup')) {
        showToast('warning', 'Popup Blocked', 'Please allow popups for this site and try again.');
      } else if (error.message?.includes('redirect')) {
        showToast('error', 'Redirect Error', 'OAuth redirect failed. Please check your configuration.');
      } else {
        showToast('error', 'Google Sign In Error', 
          error.message || 'Failed to sign in with Google. Please check your configuration.');
      }
      setLoading(false);
    }
  };
          ...
          <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700">
            <div className="mb-8 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                {isLogin ? 'Sign in to your account' : 'Get started with InvoiceAI today'}
              </p>
            </div>
