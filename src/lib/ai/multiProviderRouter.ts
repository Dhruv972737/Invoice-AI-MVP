...
export interface AIProvider {
  name: 'gemini' | 'claude' | 'deepseek' | 'chatgpt';
  enabled: boolean;
  priority: number;
  dailyQuota: number;
  usedQuota: number;
...
export class MultiProviderAIRouter {
  ...
  private async initializeProviders() {
    ...
    this.providers.set('claude', {
      ...
      priority: 2,
      dailyQuota: PROVIDER_CONFIGS.claude.quotaPerDay,
      usedQuota: await this.getProviderUsage('claude'),
      costPerToken: PROVIDER_CONFIGS.claude.costPerToken,
    ...
    try {
      ...
      switch (provider.name) {
        case 'gemini':
          ...
          break;
        case 'claude':
          response = await this.callClaude(request, provider);
          break;
        case 'deepseek':
  ...
  private async callClaude(request: AIRequest, provider: AIProvider): Promise<{data: any; tokensUsed: number}> {
    if (!provider.apiKey) throw new Error('Claude API key not configured');

    const response = await fetch(PROVIDER_CONFIGS.claude.apiEndpoint, {
      method: 'POST',
      headers: {
    ...
    return {
      data: { text, raw: data },
      tokensUsed: data.usage?.input_tokens + data.usage?.output_tokens || 1
    };
  }
